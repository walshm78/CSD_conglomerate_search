...
attribute_text_file = "attribute_list.txt" #list of cif fields to search CIF for
cif_file_locations = "cif_location_v543.gcd" #text file containing the location of one CIF file location per line
output_file_name = "output_attribute_info.csv" #location of file to write output into

### list of cif attributes to extract from CIF arhicve
            ...
            if source != None:
                try: #extract information and remove problematic characters for writing into csv (non-ascii, commmas, and \n)
                    ...
                    if len(source_list) != 0:
                                source_strip = " ".join(source).strip()
                    else:
                                source_strip = "None"
